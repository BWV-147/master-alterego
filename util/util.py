...
def get_logger(name='log', level=logging.INFO, save=True):
    ...
    if save:
        ...
        if level > logging.DEBUG:
            full_fh = RotatingFileHandler(os.path.join(log_path, f'{name}.full.log'), maxBytes=1024 * 1024, backupCount=3)
    ...
    while retry_time < 5:
        ...
        try:
            ...
            try:
                ...
                logger.warning(f'send error,type={type(e)},e={e}')
                server.quit()
        except Exception as e:
            logger.warning(f'error send mail, error={e}')
            time.sleep(5)
            logger.info(f'retry sending mail...({retry_time}/5 times)')
...
def supervise_log_time(thread: threading.Thread, secs: float = 60, mail=False, interval=10, mute=False):
    ...
    while True:
        ...
        while loops > 0:
            print(f'Or it will be force stopped after {loops * 2} seconds...', end='')
            loops -= 1
            if mute:
                time.sleep(2)
            else:
