...
class Battle(BattleBase):
    @with_goto
    def xmas_wcba(self, pre_process=False):
        """
        ...
        master.quest_name = 'Xmas-wcba'
        T.read_templates('img/xmas-wcba')
    ...
    @with_goto
    def xmas_wcba2(self, pre_process=False):
        """
        阵容: 狂兰(50NP)-CBA-CBA(s)-孔明-X-X
        ...
        # pre-processing: only configure base info
        master.quest_name = 'Xmas-wcba2'
        T.read_templates('img/xmas-wcba2')
        master.svt_names = ['狂兰', 'CBA', '孔明']
        if pre_process:
            return
    ...
    def xmas_von(self, pre_process=False):
        ...
        master.svt_names = ['狂兰', '豆爸', 'CBA']
        if pre_process:
            return

        # battle part
        master.set_card_weights([3, 1, 2])
        # ----  NP     Quick    Arts   Buster ----
        master.set_card_templates([
            [(1, 6), (2, 2), (2, 3), (1, 2)],
            [(1, 7), (1, 4), (1, 3), (1, 5)],
    ...
    @with_goto
    def xmas_von2(self, pre_process=False):
        """
        阵容: 狂兰(50NP)-豆爸(30NP)-CBA(s)-CBA-X-X
        T1: CBA(s)换下去, 魔力逆流，豆爸狂兰二连
        T2: 换人服加攻
        ...
        # pre-processing: only configure base info
        master.quest_name = 'Xmas-von2'
        T.read_templates('img/xmas-von2')
        master.svt_names = ['狂兰', '豆爸', 'CBA']
        if pre_process:
            return
    ...
    def xmas_chen(self, pre_process=False):
        ...
        master.svt_names = ['CBA', '狂兰', '陈宫']
        if pre_process:
            return

        # battle part
        master.set_card_weights([1.2, 3, 1])
        # ----  NP     Quick    Arts   Buster ----
        master.set_card_templates([
            [(1, 0), (2, 1), (1, 4), (3, 4)],
            [(1, 7), (1, 5), (1, 2), (2, 5)],
